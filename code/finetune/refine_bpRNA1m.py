...
def main(epoch, cuda_id=0):
	...
	# CNN Adapter
	model = RefineNet(embedding_dim_pretrain, ct_layer_num_pretrain, nhead_pretrain, hidden_dim=64, adapter_type='cnn')
	checkpoint = torch.load(f'checkpoints/CaTFold_pretrain.pt')
	model.load_state_dict(checkpoint, strict=False)
	model.cuda()

	start = time.time()
	log_path = 'logs/finetune/bpRNA_1m/{}_bpRNA1m_test'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
	# record result
