...
def main(epoch, cuda_id=0):
	...
	finetune_data = DataGenerator(f'data/bpRNA_1m', f'train', node_input_dim=embedding_dim_pretrain, mode='finetune')
	aug_data = DataGenerator(f'data/bpRNA_1m', f'aug', node_input_dim=embedding_dim_pretrain, mode='finetune')
	finetune_data.merge(aug_data)
	finetune_dataset = Dataset(finetune_data)
	finetune_loader = data.DataLoader(finetune_dataset, 
									   batch_size=10, 
									   num_workers=10, 
									   pin_memory=True,
									   shuffle=True,
	...
	start = time.time()
	log_path = 'logs/finetune/bpRNA_1m/{}_bpRNAnew'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
	# record result
	...
	run(model, finetune_loader, optimizer, epoch, writer)
