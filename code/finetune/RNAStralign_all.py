...
def run(model, train_loader, optimizer, train_epochs, writer):
	...
	for epoch in range(train_epochs):
		...
		if epoch+1 >= 1:
			...
			torch.save(param_save, f'{writer.logdir}/CaTFold_{epoch+1}.pt')
...
def main(epoch, cuda_id=0):
	...
	RNAStralign_train_data = DataGenerator(f'data/RNAStralign', f'train', node_input_dim=embedding_dim_pretrain, mode='finetune', family_fold=False)
